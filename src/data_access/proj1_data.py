...
class Proj1Data:
    ...
    def __init__(self) -> None:
        ...
        """
        try:
            self.mongo_client = MongoDBClient(database_name=DATABASE_NAME) # This line will trigger the class MongoDBClient which is inside the configuration -> mongo_db_connection.py
    ...
    def export_collection_as_dataframe(self, collection_name: str, database_name: Optional[str] = None) -> pd.DataFrame:
        ...
        try:
            ...
            if database_name is None:
                ...
                print(f"from the if path {collection}")
            else:
                collection = self.mongo_client[database_name][collection_name]
                print(f"from the else path {collection}")
